...
run: manifests generate fmt vet generate-certs ## Run a controller from your host.
	go run ./cmd/main.go -zap-devel -zap-stacktrace-level=panic --webhook-cert-path=$(CERTSDIR) --log-format=raw --log-level=debug --metrics-disable-auth --metrics-bind-address=:8080 --metrics-secure=false
...
.PHONY: deploy
deploy: manifests generate kustomize ## Deploy controller to the K8s cluster specified in ~/.kube/config.
	cd config/manager && $(KUSTOMIZE) edit set image controller=${IMG}
